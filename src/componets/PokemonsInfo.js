...
const PokemonsInfo = () => {
   ...
   /*      <div  className="pokemons-cardinfo" >
            <h3>{pokemons.name}</h3> 
            <img src={pokemons.sprites?.other.home.front_default} alt="" />
                <p>Exp: {pokemons.base_experience}</p>
                <p>Force: { pokemons.stats?.[0].base_stat} HP</p>
                <p>Attack: {pokemons.stats?.[1].base_stat} K </p>
                <p>Defense: {pokemons.stats?.[2].base_stat} K </p>
                <p>Speed: {pokemons.stats?.[5].base_stat} KM </p>
